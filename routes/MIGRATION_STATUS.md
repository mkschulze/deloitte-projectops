...
- **Routes in blueprints**: 67 (migrated)
- **Routes remaining**: ~28 (to migrate)
...
### tasks_bp (routes/tasks.py) - 17 routes
- ✅ `/tasks` GET (list)
- ✅ `/tasks/<id>` GET (detail)
...
### admin_bp (routes/admin.py) - 18 routes
- ✅ `/admin` GET (dashboard)
- ✅ `/admin/modules` GET
...
### api_bp (routes/api.py) - 12 routes
...
## Remaining Routes (still in app.py)

### User-Entity Permissions (6 routes)
...
- ⏳ `/api/preset-fields/<id>` GET/PUT/DELETE
...
### Phase 4a (Completed - v1.19.0)
- Created blueprint structure
- Migrated core routes (auth, main, tasks, admin, api)
- 67 routes migrated

### Phase 4b (Future)
- Migrate remaining 28 routes
- Create `routes/presets.py` for preset management
- Create `routes/exports.py` for export functionality
- Remove duplicate route definitions from app.py

### Notes
- Routes in app.py are currently superseded by blueprint routes (blueprints take precedence)
- The duplicate definitions in app.py are harmless but should be cleaned up
